...
// シェーダー読み込み
//==============================================================================
bool Shader::Load(const std::wstring& vsFile, const std::wstring& psFile)
{
	HRESULT hr;
	...
	if (FAILED(hr))
	{
		if (errorBlob)
		{
			Logger::ErrorFormat("VS compile error: %s", (char*)errorBlob->GetBufferPointer());
			errorBlob->Release();
		}
		return false;
	}
	...
	if (FAILED(hr))
	{
		if (errorBlob)
		{
			Logger::ErrorFormat("PS compile error: %s", (char*)errorBlob->GetBufferPointer());
			errorBlob->Release();
		}
		vsBlob->Release();
		return false;
...
// 入力レイアウト作成（通常モデル用）
//==============================================================================
bool Shader::CreateInputLayout(ID3DBlob* vsBlob)
{
