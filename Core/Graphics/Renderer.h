...
private:
	...
	// オフスクリーン（シーン描画先）
	static ID3D11Texture2D* m_SceneColorTex;
	static ID3D11RenderTargetView* m_SceneColorRTV;
	static ID3D11ShaderResourceView* m_SceneColorSRV;

	static ID3D11Texture2D* m_SceneDepthTex;
	static ID3D11DepthStencilView* m_SceneDepthDSV;

	// エッジ抽出シェーダ
	static ID3D11VertexShader* m_EdgeVS;
	static ID3D11PixelShader* m_EdgePS;
	static ID3D11Buffer* m_EdgeParamBuffer;
	...
	// ポストエフェクト用
	static bool CreateSceneRenderTargets();
	static bool CreateEdgePostProcess();
};
