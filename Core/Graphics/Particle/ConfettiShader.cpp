//==============================================================================
// ConfettiShader.cpp - 紙吹雪用シェーダー実装
//==============================================================================

#include "ConfettiShader.h"
...
bool ConfettiShader::Load(const std::wstring& vsFile, const std::wstring& psFile)
{
	HRESULT hr;
	...
	if (FAILED(hr))
	{
		if (errorBlob)
		{
			Logger::ErrorFormat("ConfettiShader VS error: %s", (char*)errorBlob->GetBufferPointer());
			errorBlob->Release();
		}
		return false;
	}
	...
	if (FAILED(hr))
	{
		if (errorBlob)
		{
			Logger::ErrorFormat("ConfettiShader PS error: %s", (char*)errorBlob->GetBufferPointer());
			errorBlob->Release();
		}
		vsBlob->Release();
		return false;
	...
	Logger::Info("ConfettiShader loaded");
	return true;
}

void ConfettiShader::Set()
{
