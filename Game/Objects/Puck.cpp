...
// パワーモードの設定
//==============================================================================
void Puck::SetPowerMode(bool enabled)
{
	if (!m_Trail) return;
	...
	if (enabled)
	{
		// パワー時: 赤〜オレンジ系の色
		m_Trail->SetColor(
			DirectX::XMFLOAT4(1.0f, 0.2f, 0.2f, 1.0f), // 始点: 赤
			DirectX::XMFLOAT4(1.0f, 0.6f, 0.0f, 0.5f)  // 終点: オレンジ（半透明）
		);
	}
	else
	{
		// 通常時: 青〜水色系の色（元の設定）
		m_Trail->SetColor(
			DirectX::XMFLOAT4(0.2f, 0.8f, 1.0f, 1.0f), // 始点
			DirectX::XMFLOAT4(0.0f, 0.2f, 1.0f, 0.5f)  // 終点
		);
	}
}
